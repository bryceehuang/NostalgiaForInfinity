...
# Backtesting script for HardmanV1 trading strategy
# This script for finding all possible buy signals with all pairs available
...
export STRATEGY_NAME=HardmanV1
export TIMERANGE=20250701-20251001
...
if [[ -z ${STRATEGY_NAME} ]]; then
  STRATEGY_NAME_CONFIG="HardmanV1"
else
  ...
  for EXCHANGE_RUN in ${EXCHANGE_CONFIG[*]}; do
    ...
    if [[ -f "$EXCHANGE_CONFIG_FILE" ]]; then
      ...
      echo freqtrade backtesting --export signals --eps \
        ...
        --cache none --breakdown day --dry-run-wallet 1000 --stake-amount unlimited --max-open-trades 6
      echo -e "\n\`\`\`\n\n---\n\n"
